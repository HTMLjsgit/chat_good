	...
	def create
		...
		if user_signed_in?
			...
			if request.os == "PlayStation Vita"
				Message.create(content: content, image: image, room_id: room_id, ip_id: request.op, user_id: current_user.id, username: current_user.name, login: true)
			...
			if request.os == "PlayStation Vita"
				Message.create(content: content, image: image, room_id: room_id, ip_id: request.ip, username: "名無し", login: false)
