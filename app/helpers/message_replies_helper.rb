module MessageRepliesHelper
	def message_reply_url(url,file_go,room_id, id)
		...
		if perms.include?(File.extname(file).downcase)
			...
			return link_to "ダウンロード",message_reply_download_path(room_id: room_id,id: id,url: file_go), remote: false, class: "link_download"
		end
	end
end
